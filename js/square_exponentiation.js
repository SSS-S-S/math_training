...
function testStart() {
    ...
    for (let i = 0; i < times; i++) {
        const baseNumber = getRandomInt(1, numSize);
        ...
        while (optionsSet.size < 4) {
            ...
            let wrongAns;
            do {
                wrongAnsBase = getRandomInt(1, numSize + 5);
...
function handleOptionClick(event) {
    ...
    if (selected === correct) {
        ...
        timerDisplay.textContent = `回答錯誤！答案為 ${questions[currentIndex].ans}`;
        qusConOpt.style.display = "none"
        qusTimeupCont.style.display = "block";
