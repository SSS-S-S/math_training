...
function testStart() {
    function generateQuestion(num1, num2, sym, ans) {
    ...
    for (let i = 0; i < times; i++) {
        ...
        const qa = generateQuestion(random1, random2, randomSymbol, ans);
...
function startTimerForCurrent() {
    ...
    const functionReturn = startMsCountdown(time * 1000, timerDisplay, () => {
        ...
        timerDisplay.textContent = `時間到！答案為 ${questions[currentIndex].ans}`;
...
function handleOptionClick(event) {
    ...
    const correct = questions[currentIndex - 1].ans;
    averageData.push(time - (((endTime - Date.now()) / 1000).toFixed(3)));
    if (selectedOption === correct) {
        currentCorrect++;
        nextQuestion();
    } else {
        changeDisplay([qusConOpt, "none"], [qusTimeupCont, "block"]);
        timerDisplay.textContent = `回答錯誤！答案為 ${correct}`;
        qusTimeupCont.removeEventListener("click", handleTimeupClick);
        qusTimeupCont.addEventListener("click", handleTimeupClick);
    }
}
...
function showQuestion(index) {
    ...
    qusCon.textContent = q.questionText;

    qusConOptChildren.forEach((e) => e.removeEventListener("click", handleOptionClick));

    qusConOptChildren.forEach((e, i) => {
        e.textContent = q.options[i];
...
function endTest() {
    ...
    let totalTimeValue = averageData.reduce((acc, val) => acc + Number(val), 0);
    totalTime.textContent = totalTimeValue.toFixed(3) + " 秒";

    const aver = currentCorrect > 0 ? (totalTimeValue / currentCorrect) : 0;
